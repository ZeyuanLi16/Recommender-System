...
public class Multiplication {
	...
	public static class RatingMapper extends Mapper<LongWritable, Text, Text, Text> {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			//we use ":" here, differentiate from "=" from last mapper job
		}
	}
	...
	public static class MultiplicationReducer extends Reducer<Text, Text, Text, DoubleWritable> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			//separate <movieA = relation> <user : rating>
				//every relation need to multiply every user's rating
			//multiplication two map
			...
			for(Text value: values){
				if(value.toString().contains("=")){
					...
					relationMap.put(user_rating[0], Double.parseDouble(user_rating[1]));
				}
			}
